...
PI_USER="${PI_USER:-pi}"
PI_PASS="${PI_PASS:-}"
PI_HOME="${PI_HOME:-/home/samuelhartman}"
...
if [ -z "$PI_PASS" ]; then
    echo "❌ Error: PI_PASS not set. Please set it in .env file."
    exit 1
fi
if [ -z "$CLOUDFLARE_API_TOKEN" ]; then
    echo "⚠️  Warning: CLOUDFLARE_API_TOKEN not set. Cloudflare tunnel operations may fail."
fi

echo "🚀 Deploying Annas Archive MCP Server to Raspberry Pi"
...
ANNAS_DOWNLOAD_PATH="${ANNAS_DOWNLOAD_PATH:-/home/pi/Downloads/AnnasArchive}"
...
# Annas Archive MCP Configuration
